  ...
  return (
    <div>
      <div className="page-header">
        ...
        <Link to="/oncology/add" className="btn btn-primary"><i className="fas fa-plus"></i> Add Record</Link>
      </div>
      ...
      <form className="card" style={{ marginBottom: 16 }} onSubmit={handleFilterSubmit}>
        <div className="form-grid">
          ...
          <div className="detail-field full-width">
            <label className="section-label">Current Product Scope</label>
            <p className="section-help">Oncology is currently narrowed to four cancers only: Breast, Prostate, Lung, and Colorectal. Diagnosis Confirmed, Treatment Planning, and Symptom Report are the core workflow sections.</p>
          </div>
            ...
            {items.map((r: any) => (
              <tr key={r.id}>
                ...
                <td className="actions">
                  ...
                  <Link to={`/oncology/${r.id}/edit`} className="btn btn-sm btn-warning" title="Edit"><i className="fas fa-edit"></i></Link>
                  <button onClick={() => handleDelete(r.id)} className="btn btn-sm btn-danger" title="Delete"><i className="fas fa-trash"></i></button>
                </td>
              </tr>
            ))}
