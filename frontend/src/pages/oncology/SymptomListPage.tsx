...
export default function SymptomListPage() {
  ...
  const fetchData = useCallback(async () => {
    const res = await oncologyApi.symptoms.list({ page, page_size: pageSize });
  ...
  return (
    <div>
      <div className="page-header"><h2 className="page-title"><i className="fas fa-notes-medical"></i> Symptom Report</h2><button className="btn btn-primary" onClick={() => setOpen(true)}>{open ? 'Close Form' : 'Add Symptom Report'}</button></div>
      <div className="module-intro-card symptom-card"><div><span className="eyebrow">Oncology sub-module</span><h3>Track symptoms inside the oncology workflow.</h3><p>Symptom Report belongs under Oncology. Use it to log severity, onset, progression, pain, and follow-up context for clinical review.</p></div><div className="metric-chip-row"><span className="metric-chip warning">Active symptoms</span><span className="metric-chip info">Post-treatment monitoring</span><span className="metric-chip danger">Urgent review</span></div></div>
