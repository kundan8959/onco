  ...
  return (
    <div>
      <div className="page-header">
        ...
        {canCreateClinicalData && <button className="btn btn-primary" onClick={() => setOpen(true)}><i className="fas fa-plus"></i> Add / Update Lifestyle</button>}
      </div>
            ...
            <tbody>
              {items.map((l: any) => (
                <tr key={l.patient_id}>
                  ...
                  <td>{canEditClinicalData && <button className="btn btn-sm btn-warning" onClick={() => edit(l)}><i className="fas fa-edit"></i></button>}</td>
                </tr>
              ))}
      ...
      <CrudModal open={open} title="Lifestyle Record" onClose={reset}>
        <form onSubmit={submit} className="form-grid">
          ...
          <div className="form-group"><label>Diet Type</label><select value={form.diet_type || 'regular'} onChange={(e) => setForm({ ...form, diet_type: e.target.value })}><option value="regular">Regular/Balanced</option><option value="vegetarian">Vegetarian</option><option value="vegan">Vegan</option><option value="low_carb">Low Carb</option><option value="low_fat">Low Fat</option><option value="diabetic">Diabetic Diet</option><option value="heart_healthy">Heart Healthy</option><option value="other">Other</option></select></div>
          <div className="form-group"><label>Sleep Hours (per night)</label><input type="number" step="0.1" min="0" max="24" value={form.sleep_hours || ''} onChange={(e) => setForm({ ...form, sleep_hours: e.target.value })} /></div>
          <div className="form-group"><label>Sleep Quality</label><input value={form.sleep_quality || ''} onChange={(e) => setForm({ ...form, sleep_quality: e.target.value })} placeholder="e.g. Good, Poor, Insomnia" /></div>
          <div className="form-group"><label>Stress Level</label><select value={form.stress_level || ''} onChange={(e) => setForm({ ...form, stress_level: e.target.value })}><option value="">-- Select --</option><option value="low">Low</option><option value="moderate">Moderate</option><option value="high">High</option><option value="very_high">Very High</option></select></div>
