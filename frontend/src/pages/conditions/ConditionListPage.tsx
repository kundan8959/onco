...
export default function ConditionListPage() {
  ...
  const fetchData = useCallback(async () => {
    const res = await conditionsApi.list({ page, page_size: pageSize });
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setOpen(true)}><i className="fas fa-plus"></i> Add Condition</button>
      </div>
          ...
          <tbody>
            {items.map((c: any) => (
              <tr key={c.id}>
                ...
                <td>{c.notes || '-'}</td>
                <td className="actions"><button onClick={() => handleEdit(c)} className="btn btn-sm btn-warning"><i className="fas fa-edit"></i></button><button onClick={() => handleDelete(c.id)} className="btn btn-sm btn-danger"><i className="fas fa-trash"></i></button></td>
              </tr>
            ))}
      ...
      <CrudModal open={open} title={editingId ? 'Edit Condition' : 'Add Condition'} onClose={resetForm}>
        <form onSubmit={submit} className="form-grid">
          ...
          <div className="form-group"><label>Condition</label><input value={form.condition} onChange={(e) => setForm({ ...form, condition: e.target.value })} required /></div>
          <div className="form-group"><label>Other Name</label><input value={form.condition_name_other} onChange={(e) => setForm({ ...form, condition_name_other: e.target.value })} /></div>
