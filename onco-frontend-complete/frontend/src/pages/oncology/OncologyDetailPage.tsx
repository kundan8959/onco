...
export default function OncologyDetailPage() {
  ...
  const { canEditOncologyRecord, canDeleteOncologyRecord } = usePermissions();
    ...
    <div>
      <div className="page-header">
        ...
        <div>
          ...
          {canEditOncologyRecord && <Link to={`/oncology/${id}/edit`} className="btn btn-warning ml-2">
            <i className="fas fa-edit"></i> Edit
          </Link>}
          {canDeleteOncologyRecord && <button onClick={() => { if (window.confirm('Delete?')) oncologyApi.records.delete(Number(id)).then(() => navigate('/oncology')); }} className="btn btn-danger ml-2">
