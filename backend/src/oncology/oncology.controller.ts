...
import { CreateFollowupDto, CreateOncologyRecordDto, CreatePayerDto, CreateSymptomDto, CreateTreatmentDto, PayerStatusDto, SymptomStateDto, TreatmentReadinessDto, UpdateFollowupDto, UpdateOncologyRecordDto, UpdatePayerDto, UpdateSymptomDto, UpdateTreatmentDto, CreateEpisodeDto, UpdateEpisodeDto, CancelEpisodeDto } from './dto';
...
export class OncologyController {
  ...
  findOneRecord(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    return this.svc.findOneRecord(id, req.user);
  }
  ...
  findAllFollowups(@Query() query: any, @Req() req: any) {
    return this.svc.findAllFollowups(query, req.user);
  }
  ...
  findAllSymptoms(@Query() query: any, @Req() req: any) {
    return this.svc.findAllSymptoms(query, req.user);
  }
  ...
  findAllPayer(@Query() query: any, @Req() req: any) {
    return this.svc.findAllPayer(query, req.user);
  }
  ...
  @Get('treatment-episodes')
  @ApiOperation({ summary: 'List treatment episodes' })
  findAllEpisodes(@Query() query: any, @Req() req: any) {
    return this.svc.findAllEpisodes(query, req.user);
  }

  @Get('treatment-episodes/:id')
  @ApiOperation({ summary: 'Get episode detail' })
  findOneEpisode(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    return this.svc.findOneEpisode(id, req.user);
  }

  @Post('treatment-episodes')
  @ApiOperation({ summary: 'Create treatment episode' })
  createEpisode(@Body() body: CreateEpisodeDto, @Req() req: any) {
    return this.svc.createEpisode(body, req.user);
  }

  @Patch('treatment-episodes/:id')
  @ApiOperation({ summary: 'Update treatment episode' })
  updateEpisode(@Param('id', ParseIntPipe) id: number, @Body() body: UpdateEpisodeDto, @Req() req: any) {
    return this.svc.updateEpisode(id, body, req.user);
  }

  @Delete('treatment-episodes/:id')
  @HttpCode(HttpStatus.NO_CONTENT)
  deleteEpisode(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    return this.svc.deleteEpisode(id, req.user);
  }

  @Post('treatment-episodes/:id/confirm')
  @ApiOperation({ summary: 'Confirm episode and send patient notification + email' })
  confirmEpisode(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    return this.svc.confirmEpisode(id, req.user);
  }

  @Post('treatment-episodes/:id/cancel')
  @ApiOperation({ summary: 'Cancel episode and notify patient' })
  cancelEpisode(@Param('id', ParseIntPipe) id: number, @Body() body: CancelEpisodeDto, @Req() req: any) {
    return this.svc.cancelEpisode(id, body.cancellation_reason, req.user);
  }

  @Post('treatment-episodes/:id/complete')
  @ApiOperation({ summary: 'Mark episode as completed' })
  completeEpisode(@Param('id', ParseIntPipe) id: number, @Body() body: any, @Req() req: any) {
    return this.svc.completeEpisode(id, body.notes, req.user);
  }

  @Post('treatment-episodes/:id/send-reminder')
  @ApiOperation({ summary: 'Send appointment reminder to patient (in-app + email)' })
  sendEpisodeReminder(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    return this.svc.sendEpisodeReminder(id, req.user);
  }
}
